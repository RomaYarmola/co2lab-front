import type { Metadata } from "next";
import Link from "next/link";
import { notFound } from "next/navigation";
import Container from "@/components/shared/container/Container";
import PageTitle from "@/components/shared/titles/PageTitle";
import Breadcrumbs from "@/components/shared/breadcrumbs/Breadcrumbs";
import BreadcrumbJsonLd from "@/components/shared/seo/BreadcrumbJsonLd";
import BlogListJsonLd from "@/components/shared/seo/BlogListJsonLd";
import PostCard from "@/components/blog/PostCard";
import ConsultationCTA from "@/components/shared/cta/ConsultationCTA";
import { isLocale, localizePath, type Locale } from "@/i18n/config";
import { getTranslator } from "@/i18n/server";
import { createPageMetadata } from "@/utils/createMetadata";
import { ROUTES } from "@/constants/routes";
import { fetchBlogCategories, fetchBlogPosts } from "@/lib/sanity/fetchers";
import { mapBlogCategory, mapPostCard } from "@/lib/sanity/adapters";

type Props = { params: Promise<{ locale: string }> };

export const revalidate = 300;

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale: raw } = await params;
  if (!isLocale(raw)) notFound();
  const locale: Locale = raw;
  const t = getTranslator(locale, "seo.blog");

  return createPageMetadata({
    locale,
    title: t("title"),
    description: t("description"),
    path: ROUTES.blog,
  });
}

export default async function BlogPage({ params }: Props) {
  const { locale: raw } = await params;
  if (!isLocale(raw)) notFound();
  const locale: Locale = raw;

  const t = getTranslator(locale, "blog");
  const tCommon = getTranslator(locale, "common");

  const [categoryDocs, postDocs] = await Promise.all([
    fetchBlogCategories(),
    fetchBlogPosts(),
  ]);

  const categories = categoryDocs
    .map((doc) => mapBlogCategory(doc, locale))
    .filter((category) => category.slug && category.postCount > 0);
  const posts = postDocs
    .map((doc) => mapPostCard(doc, locale))
    .filter((post) => post.slug);

  const [lead, ...rest] = posts;

  const crumbs = [
    { name: tCommon("home"), path: ROUTES.home },
    { name: t("breadcrumb") },
  ];

  return (
    <>
      <BreadcrumbJsonLd locale={locale} items={crumbs} />
      <BlogListJsonLd locale={locale} posts={posts} listName={t("heading")} />

      <section className="pt-22 pb-12 lg:pt-32 lg:pb-16">
        <Container>
          <Breadcrumbs locale={locale} items={crumbs} className="mb-5 lg:mb-8" />

          <PageTitle className="mb-3 lg:mb-5 max-w-[880px]">
            {t("heading")}
          </PageTitle>
          <p className="mb-8 max-w-[720px] text-[12px] lg:text-[16px] font-light leading-[140%] text-black/70 lg:mb-12">
            {t("intro")}
          </p>

          {categories.length > 0 && (
            <nav aria-label={t("categories")} className="mb-8 lg:mb-10">
              <ul className="flex flex-wrap gap-2">
                <li>
                  <span className="inline-flex items-center rounded-full border border-black bg-black px-4 py-2.5 text-[12px] lg:text-[14px] font-medium leading-[120%] text-white">
                    {t("allPosts")}
                  </span>
                </li>
                {categories.map((category) => (
                  <li key={category.id}>
                    <Link
                      href={localizePath(
                        locale,
                        `${ROUTES.blog}/category/${category.slug}`,
                      )}
                      className="inline-flex items-center gap-2 rounded-full border border-black/15 bg-white px-4 py-2.5 text-[12px] lg:text-[14px] font-medium leading-[120%] transition-colors duration-300 xl:hover:border-black"
                    >
                      {category.title}
                      <span className="text-[10px] font-light text-black/40">
                        {category.postCount}
                      </span>
                    </Link>
                  </li>
                ))}
              </ul>
            </nav>
          )}

          {posts.length === 0 ? (
            <div className="rounded-[18px] border border-black/10 px-6 py-14 text-center">
              <p className="mb-2 text-[16px] lg:text-[20px] font-medium uppercase leading-[120%]">
                {t("noPosts")}
              </p>
              <p className="text-[12px] lg:text-[14px] font-light leading-[140%] text-black/60">
                {t("noPostsHint")}
              </p>
            </div>
          ) : (
            <>
              {lead && (
                <div className="mb-4 lg:mb-5">
                  <PostCard post={lead} locale={locale} featured />
                </div>
              )}
              {rest.length > 0 && (
                <ul className="grid grid-cols-1 gap-4 xs:grid-cols-2 lg:grid-cols-3 lg:gap-5">
                  {rest.map((post) => (
                    <li key={post.id} className="h-full">
                      <PostCard post={post} locale={locale} />
                    </li>
                  ))}
                </ul>
              )}
            </>
          )}
        </Container>
      </section>

      <ConsultationCTA locale={locale} />
    </>
  );
}
